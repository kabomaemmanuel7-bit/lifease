  ...
  useEffect(() => {
    async function loadWorker() {
      const { data: workerProfile } = await supabase
        ...
        .select(
          "id, metier, zone, status, rating, rating_count, experience_years, competences, description"
        )
        .eq("id", params.id)
        .single();
  ...
  return (
    <main className="mx-auto max-w-md px-5 py-6">
      ...
      <div className="mb-4 flex items-center gap-4">
        <div className="h-16 w-16 shrink-0 rounded-full bg-wine-100" />
      ...
      {worker.competences.length > 0 && (
        <div className="mb-4">
          <p className="mb-2 text-sm font-medium text-ink-600">Compétences</p>
